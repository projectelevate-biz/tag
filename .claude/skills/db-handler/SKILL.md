...
2.  **Multi-Tenancy**:
    -   **MANDATORY**: If the data belongs to a tenant, add `organizationId` Foreign Key.
    -   `organizationId: text("organizationId").notNull().references(() => organizations.id, { onDelete: "cascade" })`
3.  **Columns**: Add ID (UUID), timestamps, and data columns.
    - **Mandatory**: Use Zod schema for any JSONB columns.
4.  **Relations**: Define `relations` and Foreign Keys.
5.  **Verification**: Ask the user: "Is this structure correct? Are there any missing relations?"
6.  **Migration**: 
...
- **Indexes**: You **MUST** add indexes for:
    - **Organization ID**: `index("table_org_id_idx").on(t.organizationId)` (Crucial for tenant isolation).
