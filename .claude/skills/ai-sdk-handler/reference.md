...
export const POST = withOrganizationAuthRequired(async (req, { session }) => {
  ...
  // Optional: Check credits here using organization.id
  ...
  const result = await streamText({
    ...
    async onFinish({ text, usage }) {
        // Log usage to DB for the Organization
        console.log(`Org ${organization.id} (User ${user.id}) used ${usage.totalTokens} tokens.`);
...
export const POST = withOrganizationAuthRequired(async (req, { session }) => {
  ...
  // Save to DB linked to organization.id
...
export const researchAgent = inngest.createFunction(
    ...
    async ({ event, step }) => {
        const { topic, userId, organizationId } = event.data;
