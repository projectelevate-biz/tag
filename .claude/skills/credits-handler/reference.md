...
- **Organization Balance**: Cached in `organizations.credits` (JSONB).
- **Data Types**:
...
## Best Practices
1. **Stable Core**: Rely on exported functions (`addCredits`, `useOrganization`). Avoid changing `recalculate.ts`.
2. **Idempotency**: Always provide a `paymentId` for additions.
3. **Atomic**: Never manually update `organizations.credits`. Use the helpers.
