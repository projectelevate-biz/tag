...
To show the user's current balance, use the `useOrganization` hook (Credits are per-organization).

### Key Hook: `useOrganization`
**Location**: `src/lib/organizations/useOrganization.ts`
...
export function CreditBalance() {
  ...
  return (
    <div>
      Image Credits: {organization?.credits?.image_generation || 0}
    </div>
  );
}
```
...
- **Input**: `organizationId`, `planId`, `paymentId` (for idempotency).
...
const hasBalance = canDeductCredits(
  "image_generation", 
  1, 
  organization // Must contain { credits: { ... } }
);
