...
class PayPalWebhookHandler {
  ...
  async handleSubscriptionCancelled() {
    ...
    // Handle organization plan downgrade
    if (context.organizationId) {
      // Downgrade organization to default plan
      await downgradeToDefaultPlan({ organizationId: context.organizationId });
    }
        ...
        switch (eventType) {
          ...
          case "PAYMENT.CAPTURE.COMPLETED":
            if (
              ...
              console.warn(
                "[PayPal Webhook] Skipped: resource type mismatch for order",
                { resourceType }
              );
            }
            break;
          ...
          case "BILLING.SUBSCRIPTION.RE-ACTIVATED":
            if (resourceType === "subscription") {
              console.log(
                "[PayPal Webhook] Processing subscription activated",
          ...
          case "BILLING.SUBSCRIPTION.EXPIRED":
            if (resourceType === "subscription") {
              console.log(
                "[PayPal Webhook] Processing subscription cancelled",
                { subscriptionId: event.resource?.id }
              );
