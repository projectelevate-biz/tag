...
/**
 * Allocates credits to a organization based on its new plan
 * @param params - Parameters object containing organizationId, planId, paymentId, and optional paymentMetadata
 */
export async function allocatePlanCredits(
  ...
  const { organizationId, planId, paymentId, paymentMetadata } = params;
  ...
  try {
    ...
    console.log(
      `Allocating credits for plan ${planCodename} (${planName}) to organization ${organizationId}`
    );
    ...
    for (const [creditType, creditConfig] of creditAllocations) {
      try {
        ...
        console.log(
          `Successfully allocated ${creditConfig.amount} ${creditType} credits to organization ${organizationId} for plan ${planCodename}`
        );
      } catch (error) {
        console.error(
          `Error allocating ${creditType} credits for organization ${organizationId}:`,
          error
        );
        ...
        if (
          ...
          console.log(
            `Credits allocation already processed for ${creditType} on plan ${planCodename} for organization ${organizationId}`
          );
        } else {
    ...
    console.log(
      `Completed credit allocation for plan ${planCodename} to organization ${organizationId}`
    );
  } catch (error) {
