...
export const getCreditsPrice = (
  ...
  // TIP: You can also pass the organization from both frontend and backend to get the price based on its current plan
  ...
  if (priceCalculator) {
    return priceCalculator(amount, organizationPlan);
  }
...
/**
 * Checks if a organization can deduct credits
...
export const canDeductCredits = (
  creditType: CreditType,
  amount: number,
  organization: Pick<Organization, "credits">
) => {
  const currentCredits = organization.credits?.[creditType] || 0;
