  ...
  const response = await fetch(`${PAYPAL_BASE_URL}/v2/checkout/orders`, {
    ...
    body: JSON.stringify({
      ...
      application_context: {
        ...
        user_action: "PAY_NOW",
        return_url: `${process.env.NEXT_PUBLIC_APP_URL}/app/subscribe/success?provider=paypal&codename=${plan.codename}&type=onetime&paypalContextId=${contextId}`,
        cancel_url: `${process.env.NEXT_PUBLIC_APP_URL}/app/subscribe/error?provider=paypal&codename=${plan.codename}&type=onetime&code=PAYPAL_CANCELLED&paypalContextId=${contextId}`,
      },
    }),
  });
  ...
  const response = await fetch(`${PAYPAL_BASE_URL}/v1/billing/subscriptions`, {
    ...
    body: JSON.stringify({
      ...
      application_context: {
        ...
        return_url: `${process.env.NEXT_PUBLIC_APP_URL}/app/subscribe/success?provider=paypal&codename=${plan.codename}&type=${frequency}&paypalContextId=${contextId}`,
        cancel_url: `${process.env.NEXT_PUBLIC_APP_URL}/app/subscribe/error?provider=paypal&codename=${plan.codename}&type=${frequency}&code=PAYPAL_CANCELLED&paypalContextId=${contextId}`,
      },
      payment_method: {
