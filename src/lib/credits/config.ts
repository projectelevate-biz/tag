...
export const creditsConfig: CreditsConfig = {
  ...
  video_generation: {
    ...
    currency: "USD",
    minimumAmount: 1,
    priceCalculator: (amountOfCredits, organizationPlan) => {
      // If organizationplan is provided, you can use it to calculate the price
      // For example, if organization's plan is
      // enterprise plan and organizationPlan.quotas.videoCreditsRate is 0.0001,
      // return amountOfCredits * 0.0001
      // Else use default rate of 0.01
      console.log({ organizationPlan });
...
export const onNewOrganizationCredits: OnRegisterCredits = {
  image_generation: {
    // Credit type
    amount: 50,
